...
def import_data():
    ...
    try:
        ...
        highlight_ids = set(msg['highlightId'] for msg in data.get('highlight_messages', []))
        if highlight_ids:
             db.query(DBHighlightMessage).filter(DBHighlightMessage.highlight_id.in_(highlight_ids)).delete(synchronize_session=False)
        ...
        for item in data.get('highlight_messages', []):
            ...
            db.add(message) # Use add for new messages since they likely don't have IDs in JSON
